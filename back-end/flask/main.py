...
def finishPay():
    ...
    if not (checkLogin()):
        return redirect("/")
    webhook_url = "https://discord.com/api/webhooks/1188744100850180158/FWcq55S3BRyj9ag8zhOzkJqxFDms4F49siw9Q3p435CM6YmPg-OLBAuRvUYkKEwax16e"
...
def sendSign():
    ...
    if not (checkLogin()):
        return redirect("/")
    debug.panel("sign", str(signConsent(
        userID=session["userID"], consentID=session["agreement"], signData=data)))
    return str(True)
...
def adminLoginPage():
    ...
    for item in data:
        ...
        timestamp_index = 20
        item_list[timestamp_index] = timestamp_to_date(
            float(item_list[timestamp_index]))
        if item_list[24]:
